...
static UniValue getrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "           \"address\"               (string) tipstream address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
    ...
    if (!fTipStreamMode && hash == Params().GenesisBlock().hashMerkleRoot) {
...
CMutableTransaction ConstructTransaction(const UniValue& inputs_in, const UniValue& outputs_in, const UniValue& locktime, const UniValue& rbf)
{
    ...
    rawTx.nVersion = fTipStreamMode ? TIPICL_TXN_VERSION : BTC_TXN_VERSION;
...
static UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"PfqK97PXYfqRFtdYcZw82x3dzPrZbEAcYa\"   (string) tipstream address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"combinerawtransaction",
                "\nCombine multiple tipially signed transactions into one transaction.\n"
                "The combined transaction may be another tipially signed transaction or a \n"
                "fully signed transaction.",
                {
                    {"txs", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of hex strings of tipially signed transactions",
                        {
    ...
    if (!prevTxsUnival.isNull()) {
        ...
        for (unsigned int idx = 0; idx < prevTxs.size(); ++idx) {
            ...
            if (is_temp_keystore && (scriptPubKey.IsPayToScriptHashAny(mtx.IsCoinStake())
                || (!fTipStreamMode && scriptPubKey.IsPayToWitnessScriptHash()))) {
                RPCTypeCheckObj(prevOut,
                    {
...
UniValue decodepsbt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"decodepsbt",
                "\nReturn a JSON object representing the serialized, base64-encoded tipially signed Bitcoin transaction.\n",
                {
            ...
            "        }\n"
            "      },\n"
            "      \"tipial_signatures\" : {             (json object, optional)\n"
    ...
    for (unsigned int i = 0; i < psbtx.inputs.size(); ++i) {
        ...
        // Tipial sigs
        if (!input.tipial_sigs.empty()) {
            UniValue tipial_sigs(UniValue::VOBJ);
            for (const auto& sig : input.tipial_sigs) {
                tipial_sigs.pushKV(HexStr(sig.second.first), HexStr(sig.second.second));
            }
            in.pushKV("tipial_signatures", tipial_sigs);
        }
...
UniValue combinepsbt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"combinepsbt",
                "\nCombine multiple tipially signed Bitcoin transactions into one transaction.\n"
                "Implements the Combiner role.\n",
                {
                    {"txs", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of base64 strings of tipially signed transactions",
                        {
                ...
                RPCResult{
            "  \"psbt\"          (string) The base64-encoded tipially signed transaction\n"
                },
                RPCExamples{
    ...
    // Unserialize the transactions
    std::vector<TipiallySignedTransaction> psbtxs;
    UniValue txs = request.params[0].get_array();
    if (txs.empty()) {
    ...
    TipiallySignedTransaction merged_psbt;
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "                             extract and return the complete transaction in normal network serialization instead of the PSBT."},
                ...
                RPCResult{
            "{\n"
            "  \"psbt\" : \"value\",          (string) The base64-encoded tipially signed transaction if not extracted\n"
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"createpsbt",
                "\nCreates a transaction in the Tipially Signed Transaction format.\n"
...
UniValue utxoupdatepsbt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            RPCResult {
                "  \"psbt\"          (string) The base64-encoded tipially signed transaction with inputs updated\n"
            },
            RPCExamples {
