...
static void ApplyStats(CCoinsStats &stats, CHashWriter& ss, const uint256& hash, const std::map<uint32_t, Coin>& outputs)
{
    ...
    for (const auto& output : outputs) {
        ...
        stats.nBogoSize += 32 /* txid */ + 4 /* vout index */ + 4 /* height + coinbase */ + 8 /* amount */ +
                           ...
                           + (fTipStreamMode ? 1 /* nType */ + 33 /* commitment */ : 0);
    }
    ss << VARINT(0u);
}
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of tipstream addresses\n"
...
static UniValue getchaintips(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "4.  \"valid-fork\"            This branch is not tip of the active chain, but is fully validated\n"
    ...
    for (const CBlockIndex* block : setTips)
    {
        ...
        if (chainActive.Contains(block)) {
            // This block is tip of the currently active chain.
            status = "active";
            ...
            // This block is fully validated, but no longer tip of the active chain. It was probably the active block once, but was reorganized.
            status = "valid-fork";
        } else if (block->IsValid(BLOCK_VALID_TREE)) {
            // The headers for this block are valid, but it has not been validated. It was probably never tip of the most-work chain.
