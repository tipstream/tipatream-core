...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    fTipStreamMode = !gArgs.GetBoolArg("-btcmode", false); // qa tests
    fCreateBlank = gArgs.GetBoolArg("-create", false);

    if (argc < 2 || HelpRequested(gArgs)) {
        // First tip of help message is specific to this utility
        std::string strUsage = PACKAGE_NAME " tipstream-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  tipstream-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoin transaction\n" +
            "or:     tipstream-tx [options] -create [commands]   Create hex-encoded bitcoin transaction\n" +
            "\n";
...
{
    int64_t newVersion;
    if (!ParseInt64(cmdVal, &newVersion) || newVersion < 1 || newVersion > CTransaction::MAX_STANDARD_TIPICL_VERSION)
        throw std::runtime_error("Invalid TX version requested: '" + cmdVal + "'");

    if (!tx.IsTipStreamVersion() && IsTipStreamTxVersion(newVersion)) {
...
{
    std::vector<std::string> vStrInputTips;
    boost::split(vStrInputTips, strInput, boost::is_any_of(":"));

    // separate TXID:VOUT in string
    if (vStrInputTips.size()<2)
        throw std::runtime_error("TX input missing separator");

    // extract and validate TXID
    uint256 txid;
    if (!ParseHashStr(vStrInputTips[0], txid)) {
    ...
    const std::string& strVout = vStrInputTips[1];
    int64_t vout;
    ...
    if (vStrInputTips.size() > 2)
        nSequenceIn = std::stoul(vStrInputTips[2]);
...
static void MutateTxAddWitness(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    int64_t inIdx;
    if (!ParseInt64(vStrInputTips[0], &inIdx) || inIdx < 0 || inIdx >= static_cast<int64_t>(tx.vin.size())) {
        throw std::runtime_error("Invalid TX input index '" + vStrInputTips[0] + "'");
    }

    CTxIn &txin = tx.vin[inIdx];

    for (size_t i = 1; i < vStrInputTips.size(); ++i) {
        std::vector<uint8_t> vScript;
        if (vStrInputTips[i].length() > 0) {
            if (!IsHex(vStrInputTips[i])) {
                std::string strErr = "Expecting hex input '" + vStrInputTips[i] + "'";
                throw std::runtime_error(strErr.c_str());
            }
            vScript = ParseHex(vStrInputTips[i]);
        }
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CAmount value = ExtractAndValidateValue(vStrInputTips[0]);

    // extract and validate ADDRESS
    std::string strAddr = vStrInputTips[1];
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (vStrInputTips.size() < 2 || vStrInputTips.size() > 3)
        throw std::runtime_error("TX output missing or too many separators");
    ...
    CPubKey pubkey(ParseHex(vStrInputTips[1]));
    ...
    if (vStrInputTips.size() == 3) {
        std::string flags = vStrInputTips[2];
...
static void MutateTxAddOutMultiSig(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    // Extract REQUIRED
    uint32_t required = stoul(vStrInputTips[1]);

    // Extract NUMKEYS
    uint32_t numkeys = stoul(vStrInputTips[2]);

    // Validate there are the correct number of pubkeys
    if (vStrInputTips.size() < numkeys + 3)
    ...
    for(int pos = 1; pos <= int(numkeys); pos++) {
        CPubKey pubkey(ParseHex(vStrInputTips[pos + 2]));
        if (!pubkey.IsFullyValid())
            throw std::runtime_error("invalid TX output pubkey");
    ...
    bool bSegWit = false;
    bool bScriptHash = false;
    if (vStrInputTips.size() == numkeys + 4) {
        std::string flags = vStrInputTips.back();
        bSegWit = (flags.find('W') != std::string::npos);
        bScriptHash = (flags.find('S') != std::string::npos);
    }
    else if (vStrInputTips.size() > numkeys + 4) {
...
static void MutateTxAddOutScript(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    std::string strScript = vStrInputTips[1];
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (tx.IsTipStreamVersion() && amount == 0)
...
{
    if (!tx.IsTipStreamVersion())
        throw std::runtime_error("tx not tipstream version.");
    ...
    std::string &strCommitment = vStrInputTips[0];
    ...
    std::string &strRP = vStrInputTips[2];
    if (!IsHex(strRP))
    ...
    if (vStrInputTips.size() > 3) {
        std::string &strData = vStrInputTips[3];
        if (!IsHex(strData))
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        tx.nVersion = CTransaction::CURRENT_TIPICL_VERSION;
        int startArg;
