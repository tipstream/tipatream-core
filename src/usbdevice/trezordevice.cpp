    ...
    for (;;) {
        ...
        if (req.request_type() == hw::trezor::messages::bitcoin::TxRequest::TXINPUT) {
            ...
            if (ci->second.m_shared_secret.size() == 32) {
                ...
                msg_input->set_tipstream_shared_secret(s);
            }
        ...
        if (req.request_type() == hw::trezor::messages::bitcoin::TxRequest::TXOUTPUT) {
            ...
            if (tx->vpout[i]->IsType(OUTPUT_DATA)) {
                ...
                msg_output->set_script_type(hw::trezor::messages::bitcoin::TxAck::TransactionType::TxOutputType::PAYTOTIPICLDATA);
            } else {
...
{
    return Params().NetworkIDString() == "main" ? "TipStream" : "TipStream Testnet";
};
