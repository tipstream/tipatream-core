...
{
    // Look for pubkey in all tipial sigs
...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    bool fIsP2SH = creator.IsTipStreamVersion()
...
public:
    ...
    bool is_tipstream_tx = false;
    bool IsTipStreamVersion() const override { return is_tipstream_tx; }
};
...
SignatureData DataFromTransaction(const CMutableTransaction& tx, unsigned int nIn, const std::vector<uint8_t> &amount, const CScript &scriptPubKey)
{
    ...
    extractor_checker.is_tipstream_tx = tx.IsTipStreamVersion();
...
class DummySignatureCheckerTipStream : public DummySignatureChecker
{
// IsTipStreamVersion() must return true to skip stack evaluation
public:
    DummySignatureCheckerTipStream() : DummySignatureChecker() {}
    bool IsTipStreamVersion() const override { return true; }
};
const DummySignatureCheckerTipStream DUMMY_CHECKER_TIPICL;

class DummySignatureCreatorTipStream : public DummySignatureCreator {
public:
    DummySignatureCreatorTipStream() : DummySignatureCreator(33, 32) {}
    const BaseSignatureChecker& Checker() const override { return DUMMY_CHECKER_TIPICL; }
    bool IsTipStreamVersion() const override { return true; }
};
...
const BaseSignatureCreator& DUMMY_SIGNATURE_CREATOR_TIPICL = DummySignatureCreatorTipStream();
...
bool IsSolvable(const SigningProvider& provider, const CScript& script)
{
    ...
    // if found in a transaction, we would still accept and relay that transaction. In tipicular,
