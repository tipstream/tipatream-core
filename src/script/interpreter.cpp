...
bool static IsValidSignatureEncoding(const std::vector<unsigned char> &sig) {
    ...
    // * sighash: 1-byte value indicating what data is hashed (not tip of the DER
    //   signature)
...
uint256 SignatureHash(const CScript& scriptCode, const T& txTo, unsigned int nIn, int nHashType, const std::vector<uint8_t>& amount, SigVersion sigversion, const PrecomputedTransactionData* cache)
{
    ...
    // Wrapper to serialize only the necessary tips of the transaction being signed
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    bool fIsP2SH = checker.IsTipStreamVersion() ? scriptPubKey.IsPayToScriptHashAny(checker.IsCoinStake()) : scriptPubKey.IsPayToScriptHash();
