...
// Internal function to send multitip message
static int zmq_send_multitip(void *sock, const void* data, size_t size, ...)
{
    va_list args;
    va_start(args, size);
...
bool CZMQAbstractPublishNotifier::SendMessage(const char *command, const void* data, size_t size)
{
    ...
    /* send three tips, command & data & a LE 4byte sequence number */
    unsigned char msgseq[sizeof(uint32_t)];
    WriteLE32(&msgseq[0], nSequence);
    int rc = zmq_send_multitip(psocket, command, strlen(command), data, size, msgseq, (size_t)sizeof(uint32_t), nullptr);
    if (rc == -1)
        return false;
