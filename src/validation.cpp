...
#if defined(NDEBUG)
# error "TipStream cannot be compiled without assertions."
#endif
 ...
 * other class member functions, pure functions in other tips of the consensus
...
bool CheckFinalTx(const CTransaction &tx, int flags)
{
    ...
    // transaction can be tip of the *next* block, we need to call
...
bool CheckSequenceLocks(const CTxMemPool& pool, const CTransaction& tx, int flags, LockPoints* lp, bool useExistingLockPoints)
{
    ...
    // Thus if we want to know if a transaction can be tip of the
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (ptxConflicting) {
            if (!setConflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // is for the sake of multi-tipy protocols, where we don't
                // want a single tipy to be able to disable replacement.
                //
    ...
    if (!tx.IsCoinBase())
    {
        ...
        if (fScriptChecks) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                // spent being checked as a tip of CScriptCheck.
...
void ThreadScriptCheck() {
    RenameThread("tipstream-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!fTipStreamMode  // genesis coinbase is spendable when in TipStream mode
        && fIsGenesisBlock) {
        if (!fJustCheck)
    ...
    if (!hashAssumeValid.IsNull()) {
        ...
        // This setting doesn't force the selection of any tipicular chain but makes validating some faster by
        //  effectively caching the result of tip of the verification.
    ...
    if (fTipStreamMode || VersionBitsState(pindex->pprev, consensus, Consensus::DEPLOYMENT_CSV, versionbitscache) == ThresholdState::ACTIVE) {
    ...
    blockundo.vtxundo.reserve(block.vtx.size() - (fTipStreamMode ? 0 : 1));
    ...
    if (fTipStreamMode) {
        if (block.IsProofOfStake()) { // Only the genesis block isn't proof of stake
            ...
            if (!pDevFundSettings || pDevFundSettings->nMinDevStakePercent <= 0) {
                ...
                CAmount nMinDevTip = (nCalculatedStakeReward * pDevFundSettings->nMinDevStakePercent) / 100;
                CAmount nMaxHolderTip = nCalculatedStakeReward - nMinDevTip;
                if (nMinDevTip < 0 || nMaxHolderTip < 0) {
                    return state.DoS(100, error("%s: Bad coinstake split amount (foundation=%d vs reward=%d)", __func__, nMinDevTip, nMaxHolderTip), REJECT_INVALID, "bad-cs-amount");
                }
                ...
                if (pindex->nHeight % pDevFundSettings->nDevOutputPeriod == 0) {
                    ...
                    if (outputDF->nValue < nDevBfwd + nMinDevTip) { // Max value is clamped already
                        return state.DoS(100, error("%s: Bad foundation-reward (actual=%d vs minfundtip=%d)", __func__, nStakeReward, nDevBfwd + nMinDevTip), REJECT_INVALID, "bad-cs-fund-amount");
                    }
                    ...
                    // Ensure cfwd data output is correct and nStakeReward is <= nHolderTip
                    // cfwd must == nDevBfwd + (nCalculatedStakeReward - nStakeReward) // Allowing users to set a higher split

                    if (nStakeReward < 0 || nStakeReward > nMaxHolderTip) {
                        return state.DoS(100, error("%s: Bad stake-reward (actual=%d vs maxholdertip=%d)", __func__, nStakeReward, nMaxHolderTip), REJECT_INVALID, "bad-cs-amount");
                    }
        ...
        if (block.vtx[0]->GetValueOut() > blockReward) // TipStream coins are imported as coinbase txns
...
void UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            if (fTipStreamMode)
            {
                if (pindex->nVersion > TIPICL_BLOCK_VERSION)
                    ++nUpgraded;
            } else
            {
 ...
 * tip of a single ActivateBestChainStep call.
 *
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Check that the header is valid (tipicularly PoW).  This is mostly
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        if (miSelf != mapBlockIndex.end()) {
            ...
            if (fTipStreamMode && !IsInitialBlockDownload() && state.nodeId >= 0
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (!pindex->pprev
            ...
            if (fTipStreamMode && !IsInitialBlockDownload()) {
