...
std::shared_ptr<CWallet> LoadWallet(interfaces::Chain& chain, const WalletLocation& location, std::string& error, std::string& warning)
{
    ...
    if (fTipStreamMode) {
        if (!((CHDWallet*)wallet.get())->Initialise()) {
            error = "TipStream wallet initialise failed.";
            return nullptr;
        }
...
CAmount CWalletTx::GetAvailableCredit(interfaces::Chain::Lock& locked_chain, bool fUseCache, const isminefilter& filter) const
{
    ...
    for (unsigned int i = 0; i < tx->GetNumVOuts(); i++)
    {
        ...
        {
            nCredit += fTipStreamWallet ? pwallet->GetCredit(tx->vpout[i].get(), filter)
...
bool CWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used) const
{
    ...
    if (coin_control.m_avoid_tipial_spends && vCoins.size() > OUTPUT_GROUP_MAX_ENTRIES) {
...
std::shared_ptr<CWallet> CWallet::CreateWalletFromFile(interfaces::Chain& chain, const WalletLocation& location, uint64_t wallet_creation_flags)
{
    ...
    std::shared_ptr<CWallet> walletInstance(fTipStreamMode
    ...
    if (gArgs.GetBoolArg("-upgradewallet", false) && !fTipStreamMode) {
    ...
    if (!fTipStreamMode) // Must rescan after hdwallet is loaded
...
int CMerkleTx::GetBlocksToMaturity(interfaces::Chain::Lock& locked_chain, const int *pdepth) const
{
    ...
    if (fTipStreamMode && (chainActive.Height() < COINBASE_MATURITY * 2)) {
