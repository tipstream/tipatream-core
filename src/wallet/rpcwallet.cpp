...
void EnsureWalletIsUnlocked(CWallet * const pwallet)
{
    ...
    if (IsTipStreamWallet(pwallet)
        && GetTipStreamWallet(pwallet)->fUnlockForStakingOnly)
...
static UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"getnewaddress",
                "\nReturns a new TipStream address for receiving payments.\n"
                ...
                RPCResult{
            "\"address\"    (string) The new tipstream address\n"
                },
                RPCExamples{
    ...
    if (IsTipStreamWallet(pwallet)) {
        ...
        CPubKey newKey;
        CHDWallet *phdw = GetTipStreamWallet(pwallet);
        {
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            RPCHelpMan{"getrawchangeaddress",
                "\nReturns a new TipStream address, for receiving change.\n"
...
static UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "The tipstream address to be associated with a label."},
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TipStream address");
    }
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 9)
        throw std::runtime_error(
            RPCHelpMan{"sendtoaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The tipstream address to send to."},
                    {"amount", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The amount in " + CURRENCY_UNIT + " to send. eg 0.1"},
                    {"comment", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "A comment used to store what the transaction is for.\n"
            "                             This is not tip of the transaction, just kept in your wallet."},
                    {"comment_to", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "A comment to store the name of the person or organization\n"
            "                             to which you're sending the transaction. This is not tip of the \n"
            "                             transaction, just kept in your wallet."},
                    {"subtractfeefromamount", RPCArg::Type::BOOL, /* default */ "false", "The fee will be deducted from the amount being sent.\n"
            "                             The recipient will receive less tipstream than you enter in the amount field."},
...
static UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The tipstream address\n"
        ...
        throw std::runtime_error(
            RPCHelpMan{"signmessage",
                "\nSign a message with the private key of an address" +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The tipstream address to use for the private key."},
...
static UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The tipstream address for transactions."},
    ...
    for (const std::pair<const uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        if ((!fTipStreamWallet && wtx.IsCoinBase()) || !CheckFinalTx(*wtx.tx))
            continue;
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            RPCHelpMan{"sendmany",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "A json object with addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The tipstream address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
            ...
            "                           Those recipients will receive less tipstream than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid TipStream address: ") + name_);
        }
...
static UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5) {
        std::string msg =
            RPCHelpMan{"addmultisigaddress",
                ...
                "Each key is a TipStream address or hex-encoded public key.\n"
    ...
    bool fbech32 = fTipStreamMode && request.params.size() > 3 ? request.params[3].get_bool() : false;
    bool f256Hash = fTipStreamMode && request.params.size() > 4 ? request.params[4].get_bool() : false;
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain(locked_chain) >= nMinDepth) {
        for (const COutputEntry &r : listReceived) {
            ...
            if (fTipStreamWallet
                ...
                if (GetTipStreamWallet(pwallet)->GetStealthLinked(idK, sx)) {
            ...
            if (wtx.IsCoinBase()) {
                ...
                if (wtx.IsImmatureCoinBase(locked_chain)) {
                    ...
                    entry.pushKV("category", (fTipStreamMode ? "coinbase" : "generate"));
                }
            } else {
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"address\",    (string) The tipstream address of the transaction.\n"
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"listsinceblock",
                ...
                "If \"blockhash\" is no longer a tip of the main chain, transactions from the fork point onward are included.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"address\" : \"address\",          (string) The tipstream address involved in the transaction\n"
            "      \"category\" :                      (string) The transaction category.\n"
            "                   \"send\"                  Transactions sent.\n"
            "                   \"receive\"               Non-coinbase transactions received.\n"
            "                   \"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
...
static UniValue abandontransaction(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->mapWallet.count(hash)) {
        if (!IsTipStreamWallet(pwallet) || !GetTipStreamWallet(pwallet)->HaveTransaction(hash)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid or non-wallet transaction id");
        }
    }
...
static UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending tipstream\n"
...
static UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if (fTipStreamWallet) {
        ...
        obj.pushKV("total_balance",         ValueFromAmount(
            bal.nTip + bal.nTipUnconf + bal.nTipStaked + bal.nTipImmature
            + bal.nBlind + bal.nBlindUnconf
            + bal.nAnon + bal.nAnonUnconf));

        obj.pushKV("balance",               ValueFromAmount(bal.nTip));

        obj.pushKV("blind_balance",         ValueFromAmount(bal.nBlind));
        obj.pushKV("anon_balance",          ValueFromAmount(bal.nAnon));
        obj.pushKV("staked_balance",        ValueFromAmount(bal.nTipStaked));

        obj.pushKV("unconfirmed_balance",   ValueFromAmount(bal.nTipUnconf));
        obj.pushKV("unconfirmed_blind",     ValueFromAmount(bal.nBlindUnconf));
        obj.pushKV("unconfirmed_anon",      ValueFromAmount(bal.nAnonUnconf));
        obj.pushKV("immature_balance",      ValueFromAmount(bal.nTipImmature));

        if (bal.nTipWatchOnly > 0 || bal.nTipWatchOnlyUnconf > 0 || bal.nTipWatchOnlyStaked > 0) {
            obj.pushKV("watchonly_balance",                 ValueFromAmount(bal.nTipWatchOnly));
            obj.pushKV("watchonly_staked_balance",          ValueFromAmount(bal.nTipWatchOnlyStaked));
            obj.pushKV("watchonly_unconfirmed_balance",     ValueFromAmount(bal.nTipWatchOnlyUnconf));
            obj.pushKV("watchonly_total_balance",
                ValueFromAmount(bal.nTipWatchOnly + bal.nTipWatchOnlyStaked + bal.nTipWatchOnlyUnconf));
        }
    } else {
    ...
    int nTxCount = (int)pwallet->mapWallet.size() + (fTipStreamWallet ? (int)((CHDWallet*)pwallet)->mapRecords.size() : 0);
    obj.pushKV("txcount",       (int)nTxCount);

    CKeyID seed_id;
    if (IsTipStreamWallet(pwallet)) {
        CHDWallet *pwhd = GetTipStreamWallet(pwallet);
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting tipstreamd will be"
...
static UniValue createwallet(const JSONRPCRequest& request)
{
    ...
    if (fTipStreamMode && !((CHDWallet*)wallet.get())->Initialise()) {
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            RPCHelpMan{"listunspent",
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "A json array of bitcoin addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "tipstream address"},
                        },
                    },
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the tipstream address\n"
            "    \"coldstaking_address\"  : \"address\" (string) the tipstream address this output must stake on\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid TipStream address: ") + input.get_str());
            }
...
void FundTransaction(CWallet* const pwallet, CMutableTransaction& tx, CAmount& fee_out, int& change_position, UniValue options)
{
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid tipstream address");
            }
    ...
    size_t nOutputs = IsTipStreamWallet(pwallet) ? tx.vpout.size() : tx.vout.size();
    if (nOutputs == 0)
            ...
            RPCHelpMan{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The tipstream address to receive the change"},
...
public:
    ...
    void ProcessSubScript(const CScript& subscript, UniValue& obj, bool include_addresses = false) const
    {
        ...
        // inspecting multisig tipicipants.
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nReturn information about the given tipstream address. Some information requires the address\n"
            "to be in the wallet.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The tipstream address to get the information of."},
                },
                RPCResult{
            "{\n"
            "  \"address\" : \"address\",        (string) The tipstream address validated\n"
...
UniValue sethdseed(const JSONRPCRequest& request)
{
    ...
    if (IsTipStreamWallet(pwallet))
        throw JSONRPCError(RPC_WALLET_ERROR, "Not necessary in TipStream mode.");
...
UniValue walletprocesspsbt(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "       \"SINGLE|ANYONECANPAY\""},
                ...
                RPCResult{
            "{\n"
            "  \"psbt\" : \"value\",          (string) The base64-encoded tipially signed transaction\n"
            "  \"complete\" : true|false,   (boolean) If the transaction has a complete set of signatures\n"
            "  ]\n"
            "}\n"
                },
    ...
    TipiallySignedTransaction psbtx;
    std::string error;
...
UniValue walletcreatefundedpsbt(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            RPCHelpMan{"walletcreatefundedpsbt",
                "\nCreates and funds a transaction in the Tipially Signed Transaction format. Inputs will be added if supplied inputs are not enough\n"
                "Implements the Creator and Updater roles.\n",
                {
                    ...
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "a json array with outputs (key-value pairs), where none of the keys are duplicated.\n"
                            ...
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the tipstream address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                                },
                                },
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                        {
                            {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The tipstream address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                            {"change_type", RPCArg::Type::STR, /* default */ "set by -changetype", "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL, /* default */ "false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
    ...
    // Make a blank psbt
    TipiallySignedTransaction psbtx(rawTx);
