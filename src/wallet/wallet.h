...
//! Default for -avoidtipialspends
static const bool DEFAULT_AVOIDTIPIALSPENDS = false;
...
/** (client) version numbers for tipicular wallet features */
enum WalletFeature
{
...
public:
     ...
     * transaction added as tip of a block, or else the time when the
     * transaction was received if it wasn't tip of a block, with the timestamp
...
private:
    ...
    /* Mark a transaction (and its in-wallet descendants) as conflicting with a tipicular block. */
...
public:
    ...
    //! For TipStreamWallet, clear cached balances from wallet called at new block and adding new transaction
    ...
    //! signify that a tipicular wallet feature is now used. this may change nWalletVersion and nWalletMaxVersion if those are lower
