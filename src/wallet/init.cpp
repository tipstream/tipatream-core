...
void WalletInit::AddWalletOptions() const
{
    ...
    gArgs.AddArg("-avoidtipialspends", strprintf("Group outputs by address, selecting all or none, instead of selecting on a per-output basis. Privacy is improved as an address is only used once (unless someone sends to it after spending from it), but may result in slightly higher fees as suboptimal coin selection may result due to the added limitation (default: %u)", DEFAULT_AVOIDTIPIALSPENDS), false, OptionsCategory::WALLET);
    ...
    gArgs.AddArg("-zapwallettxes=<mode>", "Delete all wallet transactions and only recover those tips of the blockchain through -rescan on startup"
...
bool LoadWallets(interfaces::Chain& chain, const std::vector<std::string>& wallet_files)
{
    for (const std::string& walletFile : wallet_files) {
        ...
        if (fTipStreamMode && !((CHDWallet*)pwallet.get())->Initialise()) {
            return false;
        }
        AddWallet(pwallet);
    }
