...
{
    gArgs.AddArg("-defaultlookaheadsize=<n>", strprintf(_("Number of keys to load into the lookahead pool per chain. (default: %u)"), N_DEFAULT_LOOKAHEAD), false, OptionsCategory::TIP_WALLET);
    gArgs.AddArg("-extkeysaveancestors", strprintf(_("On saving a key from the lookahead pool, save all unsaved keys leading up to it too. (default: %s)"), "true"), false, OptionsCategory::TIP_WALLET);
    gArgs.AddArg("-createdefaultmasterkey", strprintf(_("Generate a random master key and main account if no master key exists. (default: %s)"), "false"), false, OptionsCategory::TIP_WALLET);

    gArgs.AddArg("-staking", _("Stake your coins to support network and gain reward (default: true)"), false, OptionsCategory::TIP_STAKING);
    gArgs.AddArg("-stakingthreads", _("Number of threads to start for staking, max 1 per active wallet, will divide wallets evenly between threads (default: 1)"), false, OptionsCategory::TIP_STAKING);
    gArgs.AddArg("-minstakeinterval=<n>", _("Minimum time in seconds between successful stakes (default: 0)"), false, OptionsCategory::TIP_STAKING);
    gArgs.AddArg("-minersleep=<n>", _("Milliseconds between stake attempts. Lowering this param will not result in more stakes. (default: 500)"), false, OptionsCategory::TIP_STAKING);
    gArgs.AddArg("-reservebalance=<amount>", _("Ensure available balance remains above reservebalance. (default: 0)"), false, OptionsCategory::TIP_STAKING);
    gArgs.AddArg("-foundationdonationpercent=<n>", _("Percentage of block reward donated to the foundation fund, overridden by system minimum. (default: 0)"), false, OptionsCategory::TIP_STAKING);
    ...
    const CEKAKey *&pak, const CEKASCKey *&pasc, CExtKeyAccount *&pa, bool &isInvalid, SigVersion sigversion) const
{
    ...
    case TX_TIMELOCKED_MULTISIG:
    {
        ...
        // tipially owned (somebody else has a key that can spend
...
CAmount CHDWallet::GetLegacyBalance(const isminefilter& filter, int minDepth) const
{
    ...
    for (const auto& entry : mapWallet) {
        ...
        // treat change outputs specially, as tip of the amount debited.
...
bool CHDWallet::GetBalances(CHDWalletBalances &bal)
{
    ...
    for (const auto &item : mapWallet) {
        ...
        bal.nTipImmature += wtx.GetImmatureCredit(*locked_chain);
        ...
        if (wtx.IsCoinStake()
            ...
            bal.nTipStaked += nSpendable;
            bal.nTipWatchOnlyStaked += nWatchOnly;
        }

        if (wtx.IsTrusted(*locked_chain)) {
            bal.nTip += wtx.GetAvailableCredit(*locked_chain);
            bal.nTipWatchOnly += wtx.GetAvailableCredit(*locked_chain, true, ISMINE_WATCH_ONLY);
        } else {
            if (wtx.GetDepthInMainChain(*locked_chain) == 0 && wtx.InMempool()) {
                bal.nTipUnconf += wtx.GetAvailableCredit(*locked_chain);
                bal.nTipWatchOnlyUnconf += wtx.GetAvailableCredit(*locked_chain, true, ISMINE_WATCH_ONLY);
            }
        }
    }
            ...
            switch (r.nType) {
                ...
                case OUTPUT_STANDARD:
                    if (r.nFlags & ORF_OWNED) {
                        ...
                        if (fInMempool) {
                            bal.nTipUnconf += r.nValue;
                        }
                    } else
                    if (r.nFlags & ORF_OWN_WATCH) {
                        if (fTrusted) {
                            bal.nTipWatchOnly += r.nValue;
                        } else
                        if (fInMempool) {
                            bal.nTipWatchOnlyUnconf += r.nValue;
                        }
                    }
                    break;
                default:
    ...
    bool sign, CAmount &nFeeRet, const CCoinControl *coinControl, std::string &sError)
{
    ...
    txNew.nVersion = TIPICL_TXN_VERSION;
    txNew.vout.clear();
    ...
    int nChangePosInOut = -1;
    {
        ...
        for (;;) {
            ...
            for (const auto &coin : setCoins) {
                ...
                if (!ProduceSignature(*this, DUMMY_SIGNATURE_CREATOR_TIPICL, scriptPubKey, sigdata)) {
                ...
                for (const auto &coin : setCoins) {
                    ...
                    if (!VerifyScript(txin.scriptSig, prevPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&txConst, i, vchAmount), &serror)) {
                        if (serror == SCRIPT_ERR_INVALID_STACK_OPERATION) {
                            // Unable to sign input and verification failed (possible attempt to tipially sign).
    ...
    std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used) const
{
    ...
    std::vector<OutputGroup> groups = GroupOutputs(vCoins, !coin_control.m_avoid_tipial_spends);
...
void CHDWallet::MarkConflicted(const uint256 &hashBlock, const uint256 &hashTx)
{
    ...
    // that the block is still unknown or not yet tip of the main chain,
...
bool CHDWallet::CreateCoinStake(unsigned int nBits, int64_t nTime, int nBlockHeight, int64_t nFees, CMutableTransaction &txNew, CKey &key)
{
    ...
    if (!pDevFundSettings || pDevFundSettings->nMinDevStakePercent <= 0) {
        ...
        CAmount nDevTip = (nReward * nStakeSplit) / 100;
        nRewardOut = nReward - nDevTip;
        ...
        CAmount nDevCfwd = nDevBfwd + nDevTip;
        ...
        if (LogAcceptCategory(BCLog::POS)) {
            WalletLogPrintf("%s: Coinstake reward split %d%%, foundation %s, reward %s.\n",
                __func__, nStakeSplit, FormatMoney(nDevTip), FormatMoney(nRewardOut));
        }
    }
...
bool CHDWallet::SignBlock(CBlockTemplate *pblocktemplate, int nHeight, int64_t nSearchTime)
{
    ...
    CKey key;
    pblock->nVersion = TIPICL_BLOCK_VERSION;
...
bool IsTipStreamWallet(const CKeyStore *win)
{
    return win && dynamic_cast<const CHDWallet*>(win);
};

CHDWallet *GetTipStreamWallet(CKeyStore *win)
{
    CHDWallet *rv;
    if (!win)
...
const CHDWallet *GetTipStreamWallet(const CKeyStore *win)
{
    const CHDWallet *rv;
    if (!win)
        throw std::runtime_error("Wallet pointer is null.");
