...
static int ExtractBip32InfoV(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_SECRET_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "TipStream extended secret key");
    } else
...
static int ExtractBip32InfoP(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_PUBLIC_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "TipStream extended public key");
    } else
...
static int ManageExtKey(CStoredExtKey &sek, std::string &sOptName, std::string &sOptValue, UniValue &result, std::string &sError)
{
    ...
    if (sOptName == "active") {
        if (sOptValue.length() > 0) {
            if (tip::IsStringBoolPositive(sOptValue)) {
...
static UniValue extkey(const JSONRPCRequest &request)
{
    ...
    CHDWallet *const pwallet = GetTipStreamWallet(wallet.get());
    ...
    if (mode == "import") {
        ...
        if (fBip44) {
            ...
            if (!eKey58.IsValid(CChainParams::EXT_SECRET_KEY)
                ...
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Import failed - Key must begin with a tipstream prefix.");
            }
        }
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            RPCHelpMan{"extkeyaltversion",
                ...
                "If the provided ext_key has a Bitcoin prefix the output will be encoded with a TipStream prefix.\n"
                "If the provided ext_key has a TipStream prefix the output will be encoded with a Bitcoin prefix.\n",
                {
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"getnewextaddress",
                "\nReturns a new TipStream ext address for receiving payments." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                ...
                RPCResult{
            "\"address\"              (string) The new tipstream extended address\n"
                },
                RPCExamples{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            RPCHelpMan{"getnewstealthaddress",
                "\nReturns a new TipStream stealth address for receiving payments." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"label", RPCArg::Type::STR, /* default */ "", "If specified the key is added to the address book."},
            ...
            "           Stealth addresses with prefixes will scan only incoming stealth transactions with a matching prefix."},
                ...
                RPCResult{
            "\"address\"              (string) The new tipstream stealth address\n"
                },
                RPCExamples{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            RPCHelpMan{"reservebalance",
                "\nSet reserve amount not tipicipating in network protection.\n"
...
) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    if (!listStaked.empty()) {
        ...
        if (wtx.IsCoinBase()) {
            ...
            // Handle txns tipially funded by wallet
            if (nFee < 0) {
...
static UniValue filtertransactions(const JSONRPCRequest &request)
{
    ...
    if (!request.params[0].isNull()) {
        ...
        if (options["from"].isStr()) {
            timeFrom = tip::strToEpoch(options["from"].get_str().c_str());
        } else
        if (options["from"].isNum()) {
            timeFrom = options["from"].get_int64();
        }
        if (options["to"].isStr()) {
            timeTo = tip::strToEpoch(options["to"].get_str().c_str(), true);
        } else
        if (options["to"].isNum()) {
...
static UniValue filteraddresses(const JSONRPCRequest &request)
{
    ...
    UniValue result(UniValue::VARR);
    {
        ...
        for (it = pwallet->mapAddressBook.begin(); it != pwallet->mapAddressBook.end(); ++it) {
            ...
            if (nMatchMode) {
                if (!tip::stringsMatchI(it->second.name, sMatch, nMatchMode-1)) {
                    continue;
                }
            }
...
static UniValue manageaddressbook(const JSONRPCRequest &request)
{
    ...
    if (!address.IsValid()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, _("Invalid TipStream address."));
    }
...
static UniValue getstakinginfo(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"moneysupply\": xxxxxxx,        (numeric) the total amount of tipstream in the network\n"
            "  \"reserve\": xxxxxxx,            (numeric) the total amount of tipstream in the network\n"
        ...
        throw std::runtime_error(
            RPCHelpMan{"listunspentanon",
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "", "A json array of tipstream addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, /* default */ "", "tipstream address"},
                        },
                    },
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValidStealthAddress())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid TipStream stealth address: ")+input.get_str());
    ...
    if (request.params[0].isArray()) {
        ...
        for (size_t k = 0; k < outputs.size(); ++k) {
            ...
            if (typeOut == OUTPUT_RINGCT
                ...
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TipStream stealth address");
            }
...
static std::string SendHelp(CHDWallet *pwallet, OutputTypes typeIn, OutputTypes typeOut)
{
    ...
    rv += std::string(" tip in a") + (typeOut == OUTPUT_RINGCT || typeOut == OUTPUT_CT ? " blinded" : "") + " payment to a given address"
        + (typeOut == OUTPUT_CT ? " in anon tip": "") + ".\n";

    rv += HelpRequiringPassphrase(pwallet);

    rv +=   "\nArguments:\n"
            "1. \"address\"     (string, required) The tipstream address to send to.\n"
...
static UniValue sendtiptoblind(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetTipStreamWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
...
static UniValue sendtiptoanon(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetTipStreamWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
...
static UniValue sendblindtotip(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetTipStreamWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
...
static UniValue sendanontotip(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetTipStreamWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
            RPCHelpMan{"sendtypeto",
                "\nSend tip to multiple outputs." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"typein", RPCArg::Type::STR, RPCArg::Optional::NO, "tip/blind/anon"},
                    {"typeout", RPCArg::Type::STR, RPCArg::Optional::NO, "tip/blind/anon"},
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of json objects",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::NO, "",
                                {
                                    {"address", RPCArg::Type::STR, /* default */ "", "The tipstream address to send to."},
            ...
            "                             transaction, just kept in your wallet."},
                    ...
                    {"coin_control", RPCArg::Type::OBJ, /* default */ "", "",
                        {
                            {"changeaddress", RPCArg::Type::STR, /* default */ "", "The tipstream address to receive the change"},
            ...
            "\"txid\"              (string) The transaction id.\n"
                ...
                RPCExamples{
            HelpExampleCli("sendtypeto", "anon tip \"[{\\\"address\\\":\\\"PbpVcjgYatnkKgveaeqhkeQBFwjqR7jKBR\\\",\\\"amount\\\":0.1}]\"")
                },
            }.ToString());
...
static UniValue createrawtiptransaction(const JSONRPCRequest& request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetTipStreamWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"createrawtiptransaction",
                    ...
                    {"outputs", RPCArg::Type::ARR, /* default */ "", "a json array with outputs (key-value pairs).",
                        {
                            {"", RPCArg::Type::OBJ, /* default */ "", "",
                                {
                                    {"address", RPCArg::Type::STR, /* default */ "", "The tipstream address."},
            ...
            "}\n"
                ...
                RPCExamples{
            HelpExampleCli("createrawtiptransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" \"{\\\"address\\\":0.01}\"")
            + HelpExampleCli("createrawtiptransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" \"{\\\"data\\\":\\\"00010203\\\"}\"") +
            "\nAs a JSON-RPC call\n"
            + HelpExampleRpc("createrawtiptransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\", \"{\\\"address\\\":0.01}\"")
            + HelpExampleRpc("createrawtiptransaction", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\", \"{\\\"data\\\":\\\"00010203\\\"}\"")
                },
            }.ToString());
    ...
    rawTx.nVersion = TIPICL_TXN_VERSION;
...
static UniValue fundrawtransactionfrom(const JSONRPCRequest& request)
{
    ...
    tx.nVersion = TIPICL_TXN_VERSION;
...
static UniValue verifyrawtransaction(const JSONRPCRequest &request)
{
    ...
    // Use CTransaction for the constant tips of the
...
{ //  category              name                                actor (function)                argNames
  //  --------------------- ------------------------            -----------------------         ----------
    ...
    //sendtiptotip // normal txn
    { "wallet",             "sendtiptoblind",                  &sendtiptoblind,               {"address","amount","comment","comment_to","subtractfeefromamount","narration"} },
    { "wallet",             "sendtiptoanon",                   &sendtiptoanon,                {"address","amount","comment","comment_to","subtractfeefromamount","narration"} },

    { "wallet",             "sendblindtotip",                  &sendblindtotip,               {"address","amount","comment","comment_to","subtractfeefromamount","narration"} },
    ...
    { "wallet",             "sendanontotip",                   &sendanontotip,                {"address","amount","comment","comment_to","subtractfeefromamount","narration","ringsize","inputs_per_sig"} },
    ...
    { "rawtransactions",    "createrawtiptransaction",         &createrawtiptransaction,      {"inputs","outputs","locktime","replaceable"} },
