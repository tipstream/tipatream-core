...
const char * const BITCOIN_CONF_FILENAME = "tipstream.conf";
const char * const BITCOIN_PID_FILENAME = "tipstreamd.pid";

bool fTipStreamMode = true;
bool fTipStreamWallet = false;
ArgsManager gArgs;
    ...
    for (const auto& arg_map : m_available_args) {
        switch(arg_map.first) {
            ...
            case OptionsCategory::SMSG:
                ...
                break;
            case OptionsCategory::TIP_WALLET:
                usage += HelpMessageGroup("TipStream wallet Commands:");
                break;
            case OptionsCategory::TIP_STAKING:
...
#else
    const char* pszModule = "tipstream";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TipStream
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TipStream
    // Mac: ~/Library/Application Support/TipStream
    // Unix: ~/.tipstream
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TipStream";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TipStream";
#else
    // Unix
    return pathRet / ".tipstream";
#endif
#endif
}
...
/**
 * this function tries to make a tipicular range of a file allocated (corresponding to disk space)
...
void AllocateFileRange(FILE *file, unsigned int offset, unsigned int length) {
    ...
    nFileSize.u.LowTip = nEndPos & 0xFFFFFFFF;
    nFileSize.u.HighTip = nEndPos >> 32;
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    const int TIP_START_YEAR = 2017;

    std::string sRange = TIP_START_YEAR == COPYRIGHT_YEAR
        ? strprintf(" %i ", COPYRIGHT_YEAR)
        : strprintf(" %i-%i ", TIP_START_YEAR, COPYRIGHT_YEAR);
