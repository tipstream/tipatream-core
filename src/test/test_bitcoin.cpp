...
BasicTestingSetup::BasicTestingSetup(const std::string& chainName, bool fTipStreamModeIn)
    : m_path_root(fs::temp_directory_path() / "test_tipstream" / strprintf("%lu_%i", (unsigned long)GetTime(), (int)(InsecureRandRange(1 << 30))))
{
    fTipStreamMode = fTipStreamModeIn;
    ...
    ResetParams(chainName, fTipStreamMode);
...
TestingSetup::TestingSetup(const std::string& chainName, bool fTipStreamModeIn) : BasicTestingSetup(chainName, fTipStreamModeIn)
{
    SetDataDir("tempdir");
