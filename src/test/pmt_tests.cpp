...
class CTipialMerkleTreeTester : public CTipialMerkleTree
{
public:
...
BOOST_AUTO_TEST_CASE(pmt_test1)
{
    ...
    for (int i = 0; i < 12; i++) {
        ...
        for (int att = 1; att < 15; att++) {
            ...
            // build the tipial merkle tree
            CTipialMerkleTree pmt1(vTxid, vMatch);

            // serialize
            CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << pmt1;

            // verify CTipialMerkleTree's size guarantees
            unsigned int n = std::min<unsigned int>(nTx, 1 + vMatchTxid1.size()*nHeight);
            BOOST_CHECK(ss.size() <= 10 + (258*n+7)/8);

            // deserialize into a tester copy
            CTipialMerkleTreeTester pmt2;
            ss >> pmt2;
            ...
            for (int j=0; j<4; j++) {
                CTipialMerkleTreeTester pmt3(pmt2);
                pmt3.Damage();
...
BOOST_AUTO_TEST_CASE(pmt_malleability)
{
    ...
    CTipialMerkleTree tree(vTxid, vMatch);
    std::vector<unsigned int> vIndex;
