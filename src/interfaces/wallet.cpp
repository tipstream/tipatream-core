...
public:
    ...
    {
        if (::IsTipStreamWallet(&m_wallet))
            m_wallet_tip = GetTipStreamWallet(&m_wallet);
    }
    ...
    bool unlock(const SecureString& wallet_passphrase, bool for_staking_only) override
    {
        if (!m_wallet.Unlock(wallet_passphrase))
            return false;
        if (m_wallet_tip)
            m_wallet_tip->fUnlockForStakingOnly = for_staking_only;
        return true;
    }
    ...
    WalletTx getWalletTx(const uint256& txid) override
    {
        ...
        if (m_wallet_tip) {
            const auto mi = m_wallet_tip->mapRecords.find(txid);
            if (mi != m_wallet_tip->mapRecords.end()) {
                return MakeWalletTx(*m_wallet_tip, mi);
            }
        }
    ...
    std::vector<WalletTx> getWalletTxs() override
    {
        ...
        if (m_wallet_tip) {
            for (auto mi = m_wallet_tip->mapRecords.begin(); mi != m_wallet_tip->mapRecords.end(); mi++) {
                result.emplace_back(MakeWalletTx(*m_wallet_tip, mi));
            }
        }
        ...
        int64_t& block_time) override
    {
        ...
        if (mi == m_wallet.mapWallet.end()) {
            ...
            if (m_wallet_tip) {
                auto mi = m_wallet_tip->mapRecords.find(txid);
                if (mi != m_wallet_tip->mapRecords.end()) {
                    num_blocks = ::chainActive.Height();
                    tx_status = MakeWalletTxStatus(*locked_chain, *m_wallet_tip, mi->first, mi->second);
                    return true;
                }
            }
            return false;
        ...
        int& num_blocks) override
    {
        ...
        if (m_wallet_tip) {
            ...
            if (mi != m_wallet_tip->mapRecords.end()) {
                num_blocks = ::chainActive.Height();
                in_mempool = m_wallet_tip->InMempool(mi->first);
                order_form = {};
                tx_status = MakeWalletTxStatus(*locked_chain, *m_wallet_tip, mi->first, mi->second);
                return MakeWalletTx(*m_wallet_tip, mi);
            }
    ...
    WalletBalances getBalances() override
    {
        ...
        if (m_wallet_tip) {
            ...
            if (!m_wallet_tip->GetBalances(bal)) {
                return result;
            }

            result.balance = bal.nTip;
            result.balanceStaked = bal.nTipStaked;
            result.balanceBlind = bal.nBlind;
            result.balanceAnon = bal.nAnon;
            result.unconfirmed_balance = bal.nTipUnconf + bal.nBlindUnconf + bal.nAnonUnconf;
            result.immature_balance = bal.nTipImmature;
            result.have_watch_only = bal.nTipWatchOnly || bal.nTipWatchOnlyUnconf || bal.nTipWatchOnlyStaked;
            if (result.have_watch_only) {
                result.watch_only_balance = bal.nTipWatchOnly;
                result.unconfirmed_watch_only_balance = bal.nTipWatchOnlyUnconf;
                //result.immature_watch_only_balance = m_wallet.GetImmatureWatchOnlyBalance();
                result.balanceWatchStaked = bal.nTipWatchOnlyStaked;
            }
    ...
    CoinsList listCoins(OutputTypes nType) override
    {
        ...
        if (m_wallet_tip
            ...
            for (const auto& entry : m_wallet_tip->ListCoins(*locked_chain, nType)) {
                auto& group = result[entry.first];
                for (const auto& coin : entry.second) {
                    group.emplace_back(
                        COutPoint(coin.rtx->first, coin.i), MakeWalletTxOut(*locked_chain, *m_wallet_tip, coin.txhash, coin.rtx->second, coin.i, coin.nDepth));
                }
            }
            return result;
        }
    ...
    std::vector<WalletTxOut> getCoins(const std::vector<COutPoint>& outputs) override
    {
        ...
        for (const auto& output : outputs) {
            ...
            if (m_wallet_tip) {
                const auto mi = m_wallet_tip->mapRecords.find(output.hash);
                if (mi != m_wallet_tip->mapRecords.end()) {
                    const auto &rtx = mi->second;
                    int depth = m_wallet_tip->GetDepthInMainChain(*locked_chain, rtx.blockHash, rtx.nIndex);
                    if (depth >= 0) {
                        result.back() = MakeWalletTxOut(*locked_chain, *m_wallet_tip, output.hash, rtx, output.n, depth);
                    }
                }
            }
        }
    ...
    {
        return MakeHandler(m_wallet_tip->NotifyReservedBalanceChanged.connect(fn));
    }
    ...
    bool IsTipStreamWallet() override
    {
        return m_wallet_tip;
    }
    ...
    CAmount getReserveBalance() override
    {
        if (!m_wallet_tip)
            return 0;
        return m_wallet_tip->nReserveBalance;
    }
    ...
    bool ownDestination(const CTxDestination &dest) override
    {
        if (!m_wallet_tip)
            return false;
        return m_wallet_tip->HaveAddress(dest);
    }
    ...
    bool isUnlockForStakingOnlySet() override
    {
        if (!m_wallet_tip)
            return false;
        return m_wallet_tip->fUnlockForStakingOnly;
    }
    ...
    CAmount getAvailableAnonBalance(const CCoinControl& coin_control) override
    {
        if (!m_wallet_tip)
            return 0;
        return m_wallet_tip->GetAvailableAnonBalance(&coin_control);
    }
    ...
    CAmount getAvailableBlindBalance(const CCoinControl& coin_control) override
    {
        if (!m_wallet_tip)
            return 0;
        return m_wallet_tip->GetAvailableBlindBalance(&coin_control);
    }

    CHDWallet *getTipStreamWallet() override
    {
        return m_wallet_tip;
    }
    ...
    bool setReserveBalance(CAmount nValue) override
    {
        if (!m_wallet_tip)
            return false;
        return m_wallet_tip->SetReserveBalance(nValue);
    }
    ...
    bool setUnlockedForStaking() override
    {
        if (!m_wallet_tip)
            return false;
        if (m_wallet_tip->IsLocked())
            return false;
        m_wallet_tip->fUnlockForStakingOnly = true;
        return true;
    };
    ...
    bool isDefaultAccountSet() override
    {
        if (!m_wallet_tip)
            return false;
        return (!m_wallet_tip->idDefaultAccount.IsNull());
    }
    ...
    CAmount getCredit(const CTxOutBase *txout, isminefilter filter) override
    {
        if (!m_wallet_tip)
            return 0;
        return m_wallet_tip->GetCredit(txout, filter);
    }
    ...
    isminetype txoutIsMine(const CTxOutBase *txout) override
    {
        if (!m_wallet_tip)
            return ISMINE_NO;
        return m_wallet_tip->IsMine(txout);
    }

    CHDWallet *m_wallet_tip = nullptr;
};
