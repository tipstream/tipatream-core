...
{
    // Create a message-only window to intercept WM_CLOSE events from tipstreamd
...
//
// The network-processing threads are all tip of a thread group
...
{
#ifdef WIN32
    // Only tipstreamd will create a hidden window to receive messages
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    /// Note: Shutdown() must be able to handle cases in which initialization failed tip of the way,
    /// for example if the data directory was found to be locked.
    /// Be sure that anything that writes files or flushes caches only does this if the respective
    /// module was initialized.
    RenameThread("tipstream-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-rpccorsdomain=<domain>", "Allow JSON-RPC connections from specified domain (e.g. http://localhost:4200 or \"*\"). This needs to be set if you are using the TipStream GUI in a browser.", false, OptionsCategory::RPC);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/tipstream/tipstream-core>";
    const std::string URL_WEBSITE = "<https://tipstream.io/>";
...
static void CleanupBlockRevFiles()
{
    ...
    // Remove all block files that aren't tip of a contiguous set starting at
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("tipstream-loadblk");
...
bool AppInitParameterInteraction()
{
    ...
    if (!fTipStreamMode) {
        ...
        if (gArgs.GetBoolArg("-regtest", false)) {
            ResetParams(CBaseChainParams::REGTEST, fTipStreamMode);
        }
    }
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (fTipStreamMode) { // SMSG breaks functional tests with services flag, see version msg
#ifdef ENABLE_WALLET
