...
static CBlock CreateGenesisBlockRegTest(uint32_t nTime, uint32_t nNonce, uint32_t nBits)
{
    ...
    genesis.nBits    = nBits;
    genesis.nNonce   = nNonce;
    genesis.nVersion = TIPICL_BLOCK_VERSION;
    genesis.vtx.push_back(MakeTransactionRef(std::move(txNew)));
...
static CBlock CreateGenesisBlockTestNet(uint32_t nTime, uint32_t nNonce, uint32_t nBits)
{
    ...
    // Reserved TipStream for primary round
...
public:
    CMainParams() {
        ...
        vSeeds.emplace_back("mainnet-seed.tipstream.io");
        vSeeds.emplace_back("dnsseed-mainnet.tipstream.io");
        vSeeds.emplace_back("mainnet.tipstream.io");
        ...
        base58Prefixes[PUBKEY_ADDRESS]     = {0x66}; // T
...
public:
    CTestNetParams() {
        ...
        vSeeds.emplace_back("testnet-seed.tipstream.io");
        vSeeds.emplace_back("dnsseed-testnet.tipstream.io");

        vDevFundSettings.push_back(std::make_pair(0, DevFundSettings("rTvv9vsbu269mjYYEecPYinDG8Bt7D86qD", 10, 60)));

        base58Prefixes[PUBKEY_ADDRESS]     = {0x127}; // t
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        base58Prefixes[PUBKEY_ADDRESS]     = {0x66}; // p
        base58Prefixes[SCRIPT_ADDRESS]     = {0x7a};
        base58Prefixes[PUBKEY_ADDRESS_256] = {0x77};
        base58Prefixes[SCRIPT_ADDRESS_256] = {0x7b};
        base58Prefixes[SECRET_KEY]         = {0x2e};
...
void ResetParams(std::string sNetworkId, bool fTipStreamModeIn)
{
