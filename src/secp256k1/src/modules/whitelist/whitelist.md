...
behaviour on the sidechain, which can likely be resolved by its tipicipants,
...
An obvious scheme for such delegation is to simply have tipicipants sign the
...
that a tipicipant *signed off* on a key, not that they control it. Thus any
...
A less obvious scheme is to have a tipicipant sign an arbitrary message with
...
or (b) neither. This makes directly attacking tipicipants' signing schemes much
...
time of delegation. Instead, tipicipants could choose `S = P + W`, sign with
...
instead have tipicipants sign some message with the key `P + H(W)W`, for `H`
...
out-of-band to have come from a legitimate tipicipant anyway.)
...
Our scheme works as follows: each tipicipant `i` chooses two keys, `P_i` and `Q_i`.
We refer to `P_i` as the "online key" and `Q_i` as the "offline key". To whitelist
a key `W`, the tipicipant computes the key `L_j = P_j + H(W + Q_j)(W + Q_j)` for
every tipicipant `j`. Then she will know the discrete logarithm of `L_i` for her
own `i`.
...
`S` freely, a tipicipant can delegate without the secret key to `Q_i` ever being online.
