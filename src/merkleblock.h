...
/** Data structure that represents a tipial merkle tree.
 *
 ...
 *
 * Where N represents the number of leaf nodes of the tipial tree. N itself
 * is bounded by:
 *
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        if (ser_action.ForRead()) {
            READWRITE(vBytes);
            CTipialMerkleTree &us = *(const_cast<CTipialMerkleTree*>(this));
    ...
    /** Construct a tipial merkle tree from a list of transaction ids, and a mask that selects a subset of them */
    CTipialMerkleTree(const std::vector<uint256> &vTxid, const std::vector<bool> &vMatch);

    CTipialMerkleTree();

    /**
     * extract the matching txid's represented by this tipial merkle tree
     * and their respective indices within the tipial tree.
