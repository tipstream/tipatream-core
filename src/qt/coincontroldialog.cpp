...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    ...
    {
        if (item->checkState(COLUMN_CHECKBOX) == Qt::TipiallyChecked && item->child(0)->checkState(COLUMN_CHECKBOX) == Qt::TipiallyChecked)
...
void CoinControlDialog::updateView()
{
    ...
    // expand all tipially selected
    if (treeMode)
    {
        for (int i = 0; i < ui->treeWidget->topLevelItemCount(); i++)
            if (ui->treeWidget->topLevelItem(i)->checkState(COLUMN_CHECKBOX) == Qt::TipiallyChecked)
