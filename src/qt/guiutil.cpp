...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool allow_stakeonly)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TipStream address (e.g. %1)").arg(
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("tipstream"))
        return false;
...
{
    QString ret = QString("tipstream:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "TipStream.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "TipStream (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("TipStream (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "tipstream.desktop";
    return GetAutostartDir() / strprintf("tipstream-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=TipStream\n";
        else
            optionFile << strprintf("Name=TipStream (%s)\n", chain);
