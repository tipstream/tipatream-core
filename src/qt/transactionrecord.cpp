...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const interfaces::WalletTx& wtx)
{
            ...
            if (r.vPath.size() > 0) {
                if (r.vPath[0] == ORA_STEALTH) {
                    if (r.vPath.size() < 5) {
                        ...
                        CStealthAddress sx;
                        if (wtx.tipWallet->GetStealthByIndex(sidx, sx))
                            address = sx;
                    }
                }
            } else {
    ...
    } else
    {
        ...
        if (fAllFromMe && fAllToMe)
        {
            ...
            tips.append(TransactionRecord(hash, nTime, TransactionRecord::SendToSelf, "",
                            -(nDebit - nChange), nCredit - nChange));
            tips.last().involvesWatchAddress = involvesWatchAddress;   // maybe pass to TransactionRecord as constructor argument
        } else
        if (fAllFromMe)
        {
            //
            ...
            for (unsigned int nOut = 0; nOut < wtx.tx->vpout.size(); nOut++)
            {
                ...
                {
                    // Ignore tips sent to self, as this is usually the change
        ...
        else
        {
            ...
            //
            tips.append(TransactionRecord(hash, nTime, TransactionRecord::Other, "", nNet, 0));
            tips.last().involvesWatchAddress = involvesWatchAddress;
        }
    }
