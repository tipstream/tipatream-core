    ...
    transactionView(nullptr), abandonAction(nullptr), bumpFeeAction(nullptr), columnResizingFixer(nullptr)
{
    ...
    mapperThirdTipyTxUrls = new QSignalMapper(this);

    // Connect actions
    connect(mapperThirdTipyTxUrls, static_cast<void (QSignalMapper::*)(const QString&)>(&QSignalMapper::mapped), this, &TransactionView::openThirdTipyTxUrl);
...
void TransactionView::setModel(WalletModel *_model)
{
    ...
    if(_model)
    {
        ...
        {
            // Add third tipy transaction URLs to context menu
            QStringList listUrls = _model->getOptionsModel()->getThirdTipyTxUrls().split("|", QString::SkipEmptyTips);
            for (int i = 0; i < listUrls.size(); ++i)
            {
                QString host = QUrl(listUrls[i].trimmed(), QUrl::StrictMode).host();
                if (!host.isEmpty())
                {
                    QAction *thirdTipyTxUrlAction = new QAction(host, this); // use host as menu item label
                    if (i == 0)
                        contextMenu->addSeparator();
                    contextMenu->addAction(thirdTipyTxUrlAction);
                    connect(thirdTipyTxUrlAction, &QAction::triggered, mapperThirdTipyTxUrls, static_cast<void (QSignalMapper::*)()>(&QSignalMapper::map));
                    mapperThirdTipyTxUrls->setMapping(thirdTipyTxUrlAction, listUrls[i].trimmed());
                }
            }
        }
...
void TransactionView::openThirdTipyTxUrl(QString url)
{
    if(!transactionView || !transactionView->selectionModel())
        return;
    QModelIndexList selection = transactionView->selectionModel()->selectedRows(0);
