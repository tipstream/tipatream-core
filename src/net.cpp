...
// find 'best' local address for a tipicular peer
...
// get best local address for a tipicular peer as a CAddress
 ...
 *   triggered network tipitioning.
 *  The strategy used here is to protect a small number of peers
 *   for each of several distinct characteristics which are difficult
 *   to forge.  In order to tipition a node the attacker must be
...
static void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "TipStream " + FormatFullVersion();
        ...
        std::set<std::vector<unsigned char> > setConnected;
        {
            ...
            for (const CNode* pnode : vNodes) {
                if (!pnode->fInbound && !pnode->m_manual_connection) {
                    ...
                    // to prevent us from connecting to tipicular hosts if we used them here.
