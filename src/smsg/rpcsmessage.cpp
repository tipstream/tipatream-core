        ...
        if (mode == "all"
            ...
            while (dbInbox.NextSmesg(it, sPrefix, chKey, smsgStored)) {
                ...
                if (rv == 0) {
                    ...
                    if (filter.size() > 0
                        && !(tip::stringsMatchI(msg.sFromAddress, filter, 3) ||
                            tip::stringsMatchI(sAddrTo, filter, 3) ||
                            tip::stringsMatchI(sText, filter, 3))) {
                        continue;
                    }
    ...
    if (mode == "stats") {
        ...
        uint64_t nBytes = 0;
        {
            ...
            for (it = smsgModule.buckets.begin(); it != smsgModule.buckets.end(); ++it) {
                ...
                objM.pushKV("last changed", tip::GetTimeString(it->second.timeChanged, cbuf, sizeof(cbuf)));
                ...
                if (!boost::filesystem::exists(fullPath)) {
                    ...
                    try {
                        ...
                        nBytes += nFBytes;
                        objM.pushKV("file size", tip::BytesReadable(nFBytes));
        ...
        objM.pushKV("size", tip::BytesReadable(nBytes));
...
static UniValue smsgview(const JSONRPCRequest &request)
{
            ...
            if (checkValid.IsValid()) {
...

                {
                    ...
                    for (itl = smsgModule.pwallet->mapAddressBook.begin(); itl != smsgModule.pwallet->mapAddressBook.end(); ++itl) {
                        if (tip::stringsMatchI(itl->second.name, sTemp, matchType)) {
    ...
    while (i < request.params.size()) {
        ...
        if (sTemp == "-from") {
            ...
            i++;
            sTemp = request.params[i].get_str();
            tFrom = tip::strToEpoch(sTemp.c_str());
            if (tFrom < 0) {
        ...
        if (sTemp == "-to") {
            ...
            i++;
            sTemp = request.params[i].get_str();
            tTo = tip::strToEpoch(sTemp.c_str());
            if (tTo < 0) {
    ...
    if (tFrom > 0) {
        result.pushKV("from", tip::GetTimeString(tFrom, cbuf, sizeof(cbuf)));
    }
    if (tTo > 0) {
        result.pushKV("to", tip::GetTimeString(tTo, cbuf, sizeof(cbuf)));
    }
#else
    UniValue result(UniValue::VOBJ);
    throw JSONRPCError(RPC_MISC_ERROR, "No wallet.");
