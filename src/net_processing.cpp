...
namespace {
    ...
    struct QueuedBlock {
        ...
        std::unique_ptr<TipiallyDownloadedBlock> tipialBlock;  //!< Optional, used for CMPCTBLOCK downloads
    };
...
static bool MarkBlockAsInFlight(NodeId nodeid, const uint256& hash, const CBlockIndex* pindex = nullptr, std::list<QueuedBlock>::iterator** pit = nullptr) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    std::list<QueuedBlock>::iterator it = state->vBlocksInFlight.insert(state->vBlocksInFlight.end(),
            {hash, pindex, pindex != nullptr, std::unique_ptr<TipiallyDownloadedBlock>(pit ? new TipiallyDownloadedBlock(&mempool) : nullptr)});
    state->nBlocksInFlight++;
...
static void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        ...
        // already tip of our chain (and therefore don't need it even if pruned).
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // set nodes not relaying blocks and tx and not serving (tips) of the historical blockchain as "clients"
    ...
    if (strCommand == NetMsgType::ADDR) {
        ...
        for (CAddress& addr : vAddr)
        {
            ...
            // tip because we may make feeler connections to them.
        ...
        if (pindex->nHeight <= chainActive.Height() + 2) {
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (!MarkBlockAsInFlight(pfrom->GetId(), pindex->GetBlockHash(), pindex, &queuedBlockIt)) {
                    if (!(*queuedBlockIt)->tipialBlock)
                        (*queuedBlockIt)->tipialBlock.reset(new TipiallyDownloadedBlock(&mempool));
                    else {
                ...
                TipiallyDownloadedBlock& tipialBlock = *(*queuedBlockIt)->tipialBlock;
                ReadStatus status = tipialBlock.InitData(cmpctblock, vExtraTxnForCompact);
                ...
                for (size_t i = 0; i < cmpctblock.BlockTxCount(); i++) {
                    if (!tipialBlock.IsTxAvailable(i))
                ...
                TipiallyDownloadedBlock tempBlock(&mempool);
    ...
    if (strCommand == NetMsgType::BLOCKTXN && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        bool fBlockRead = false;
        {
            ...
            if (it == mapBlocksInFlight.end() || !it->second.second->tipialBlock ||
                    it->second.first != pfrom->GetId()) {
                LogPrint(BCLog::NET, "Peer %d sent us block transactions for block we weren't expecting\n", pfrom->GetId());
                return true;
            }

            TipiallyDownloadedBlock& tipialBlock = *it->second.second->tipialBlock;
            ReadStatus status = tipialBlock.FillBlock(*pblock, resp.txn);
            if (status == READ_STATUS_INVALID) {
