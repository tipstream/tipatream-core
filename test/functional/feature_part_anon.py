...
class AnonTest(TipStreamTestFramework):
    ...
    def run_test(self):
        ...
        txnHash = nodes[0].sendtiptoanon(sxAddrTo1_1, 1, '', '', False, 'node0 -> node1 p->a')
        txnHashes.append(txnHash)

        txnHash = nodes[0].sendtiptoblind(sxAddrTo0_1, 1000, '', '', False, 'node0 -> node0 p->b')
        txnHashes.append(txnHash)
        ...
        for k in range(4):
            txnHash = nodes[0].sendtiptoanon(sxAddrTo1_1, 10, '', '', False, 'node0 -> node1 p->a')
            txnHashes.append(txnHash)
        for k in range(10):
        ...
        ro = nodes[1].sendtypeto('anon', 'tip', outputs, 'comment_to', 'comment_from', 4, 32, True)
        assert(ro['bytes'] > 0)

        txnHash = nodes[1].sendtypeto('anon', 'tip', outputs)
        txnHashes = [txnHash,]
