...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-5, "Invalid TipStream address", self.nodes[0].createrawtransaction, [], {'foo': 0})
        ...
        rawTx = self.nodes[2].createrawtransaction(inputs, outputs)
        rawTxTipialSigned = self.nodes[1].signrawtransactionwithwallet(rawTx, inputs)
        assert_equal(rawTxTipialSigned['complete'], False) #node1 only has one key, can't comp. sign the tx
        ...
        rawTxTipialSigned1 = self.nodes[1].signrawtransactionwithwallet(rawTx2, inputs)
        self.log.debug(rawTxTipialSigned1)
        assert_equal(rawTxTipialSigned1['complete'], False) #node1 only has one key, can't comp. sign the tx

        rawTxTipialSigned2 = self.nodes[2].signrawtransactionwithwallet(rawTx2, inputs)
        self.log.debug(rawTxTipialSigned2)
        assert_equal(rawTxTipialSigned2['complete'], False) #node2 only has one key, can't comp. sign the tx
        rawTxComb = self.nodes[2].combinerawtransaction([rawTxTipialSigned1['hex'], rawTxTipialSigned2['hex']])
        ...
        """
        # Collides with tipstream version
