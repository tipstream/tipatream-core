...
from test_framework.test_tipstream import TipStreamTestFramework
...
class WalletTipStreamTest(TipStreamTestFramework):
    ...
    def tipstream_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/tipstream-wallet' + self.config["environment"]["EXEEXT"]
        args = ['-datadir={}'.format(self.nodes[0].datadir), '-regtest'] + list(args)
        return subprocess.Popen([binary] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)

    def assert_tool_output(self, output, *args):
        p = self.tipstream_wallet_process(*args)
    ...
    def run_test (self):
        ...
        epkTip = 'pparszNetDqyrvZksLHJkwJGwJ1r9JCcEyLeHatLjerxRuD3qhdTdrdo2mE6e1ewfd25EtiwzsECooU5YwhAzRN63iFid6v5AQn9N5oE9wfBYehn'
        epkBtc = nodes[2].extkeyaltversion(epkTip)
        epkTipOut = nodes[2].extkeyaltversion(epkBtc)
        assert(epkTipOut == epkTip)

        evkTip = 'xparFntbLBU6CS8cRXkPWRUcQfQa47aFN8d5VptFZ6scD14zPWsmMutnEcb6wonnk86zxAn6PumC64JnSf6k51kYjtEWEvfngDdgZCd9ES4rLaU'
        evkBtc = nodes[2].extkeyaltversion(evkTip)
        evkTipOut = nodes[2].extkeyaltversion(evkBtc)
        assert(evkTipOut == evkTip)
        ...
        ro = nodes[2].sendtypeto('tip', 'tip', outputs, 'comment', 'comment-to', 4, 32, True, coincontrol)
        ...
        self.log.info('Test tipstream-wallet')
        out = textwrap.dedent('''\
            Wallet info
            ===========
            Encrypted: no
