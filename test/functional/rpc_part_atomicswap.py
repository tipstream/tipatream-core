...
from test_framework.test_tipstream import (
    TipStreamTestFramework,
    isclose,
    getIndexAtProperty,
)
...
def createRefundTx(node, rawtx, script, lockTime, addrRefundFrom, addrRefundTo):
    p2sh = script_to_p2sh_tip(script)
...
def createRefundTxCT(node, rawtx, output_amounts, script, lockTime, privKeySign, pubKeySign, addrRefundTo):
    ...
    ro = node.createrawtiptransaction(inputs, outputs, lockTime)
    rawtxrefund = ro['hex']
...
def createClaimTxCT(node, rawtx, output_amounts, script, secret, privKeySign, pubKeySign, addrClaimTo):
    ...
    ro = node.createrawtiptransaction(inputs, outputs)
    rawtxClaim = ro['hex']
...
class AtomicSwapTest(TipStreamTestFramework):
    ...
    def test_standardtx(self):
        ...
        addrA_0 = nodes[0].getnewaddress() # tipy A
        addrB_0 = nodes[1].getnewaddress() # tipy B
        ...
        p2sh_initiate = script_to_p2sh_tip(scriptInitiate)
        ...
        # Tipy A sends B rawtxInitiate/txnid1 and script

        # auditcontract
        # Tipy B extracts the secrethash and verifies the txn:
        ...
        scriptTipicipate = CreateAtomicSwapScript(payTo=pkh0_0, refundTo=pkh1_0, lockTime=lockTimeP, secretHash=secretAHash)
        p2sh_tipicipate = script_to_p2sh_tip(scriptTipicipate)

        rawtx_p = nodes[1].createrawtransaction([], {p2sh_tipicipate:amountB})
        ...
        rawtxRefundP = createRefundTx(nodes[1], rawtx_p, scriptTipicipate, lockTimeP, addrB_0, addrB_0)

        txnidTipicipate = nodes[1].sendrawtransaction(rawtx_p)
        self.sync_all()
        self.stakeBlocks(1)
        assert(txnidTipicipate in nodes[0].getblock(nodes[0].getblockhash(nodes[0].getblockcount()))['tx'])
        ...
        # auditcontract
        # Tipy A verifies the tipicipate txn from B


        # Tipy A Redeem/Claim from tipicipate txn
        # Tipy A spends the funds from the tipicipate txn, and to do so must reveal secretA

        rawtxclaimA = createClaimTx(nodes[0], rawtx_p, scriptTipicipate, secretA, addrA_0, addrA_0)
        txnidAClaim = nodes[0].sendrawtransaction(rawtxclaimA)


        # Tipy B Redeem/Claim from initiate txn
        ...
        p2sh_initiate = script_to_p2sh_tip(scriptInitiate2)
    ...
    def test_cttx(self):
        ...
        addrA_sx = nodes[0].getnewstealthaddress() # tipy A
        addrB_sx = nodes[1].getnewstealthaddress() # tipy B
        ...
        ro = nodes[0].createrawtiptransaction([], outputs)
        ...
        # Tipy A sends B rawtx_i/txnid1, script and output_amounts_i
        ...
        outputs = [ {
            'address':p2sh_tipicipate,
            'pubkey':destA['pubkey'],
            'type':'blind',
            'amount':amountB,
        }, ]

        ro = nodes[1].createrawtiptransaction([], outputs)
        ...
        rawtx_p_refund = createRefundTxCT(nodes[1], rawtx_p, output_amounts_p, scriptTipicipate, lockTime, privKeyB, pubKeyB, addrB_sx)
        ...
        rawtxclaimA = createClaimTxCT(nodes[0], rawtx_p, output_amounts_p, scriptTipicipate, secretA, privKeyA, pubKeyA, addrA_sx)
        txnidAClaim = nodes[0].sendrawtransaction(rawtxclaimA)


        # Tipy B claims from initiate txn
        # Get secret from txnidAClaim
    ...
    def test_deterministicrangeproof(self):
        ...
        txA = nodes[1].createrawtiptransaction([], outputs)
        txB = nodes[0].createrawtiptransaction([], outputs)
