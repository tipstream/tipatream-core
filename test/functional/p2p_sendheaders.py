...
Tip 1: No headers announcements before "sendheaders"
...
Tip 2: After "sendheaders", headers announcements should generally work.
...
Tip 3: Headers announcements stop after large reorg and resume after getheaders or inv from peer.
...
Tip 4: Test direct fetch behavior
...
Tip 5: Test handling of headers that don't connect.
a. Repeat 10 times:
...
class SendHeadersTest(BitcoinTestFramework):
    ...
    def test_nonnull_locators(self, test_node, inv_node):
        ...
        self.log.info("Tip 1: headers don't start before sendheaders message...")
        for i in range(4):
            self.log.debug("Tip 1.{}: starting...".format(i))
            old_tip = tip
        ...
        self.log.info("Tip 1: success!")
        self.log.info("Tip 2: announce blocks with headers after sendheaders message...")
        # TIP 2
        ...
        for i in range(10):
            self.log.debug("Tip 2.{}: starting...".format(i))
            # Mine i blocks, and alternate announcing either via
            # inv (of tip) or via headers. After each, new blocks
            # mined by the node should successfully be announced
            # with block header, even though the blocks are never requested
            for j in range(2):
                self.log.debug("Tip 2.{}.{}: starting...".format(i, j))
                blocks = []
                for b in range(i + 1):
        ...
        self.log.info("Tip 2: success!")

        self.log.info("Tip 3: headers announcements can stop after large reorg, and resume after headers/inv from peer...")

        # TIP 3.  Headers announcements can stop after large reorg, and resume after
        # getheaders or inv from peer.
        for j in range(2):
            self.log.debug("Tip 3.{}: starting...".format(j))
            ...
            for i in range(3):
                self.log.debug("Tip 3.{}.{}: starting...".format(j, i))
        ...
        self.log.info("Tip 3: success!")

        self.log.info("Tip 4: Testing direct fetch behavior...")
        tip = self.mine_blocks(1)
        height = self.nodes[0].getblockcount() + 1
        last_time = self.nodes[0].getblock(self.nodes[0].getbestblockhash())['time']
        block_time = last_time + 1
        ...
        self.log.info("Tip 4: success!")

        # Now deliver all those blocks we announced.
        [test_node.send_message(msg_block(x)) for x in blocks]

        self.log.info("Tip 5: Testing handling of unconnecting headers")
        # First we test that receipt of an unconnecting header doesn't prevent
        # chain sync.
        for i in range(10):
            self.log.debug("Tip 5.{}: starting...".format(i))
            test_node.last_message.pop("getdata", None)
            blocks = []
