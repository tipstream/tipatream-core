...
def main():
    ...
    for f in args.file:
        for function_call_str in parse_function_calls(args.function_name, f.read()):
            tips = parse_function_call_and_arguments(args.function_name, function_call_str)
            relevant_function_call_str = unescape("".join(tips))[:512]
            if (f.name, relevant_function_call_str) in FALSE_POSITIVES:
                continue
            if len(tips) < 3 + args.skip_arguments:
                exit_code = 1
                print("{}: Could not parse function call string \"{}(...)\": {}".format(f.name, args.function_name, relevant_function_call_str))
                continue
            argument_count = len(tips) - 3 - args.skip_arguments
            format_str = parse_string_content(tips[1 + args.skip_arguments])
