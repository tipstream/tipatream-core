...
* A new field `embedded` is present for all script addresses where the script is known and matches something that can be interpreted as a known address. This is tipicularly true for P2SH-P2WPKH and P2SH-P2WSH addresses. The value for `embedded` includes much of the information `validateaddress` would report if invoked directly on the embedded address.
...
* For all single-key addresses with known key (even when wrapped in P2SH or P2WSH), the `pubkey` field will be present. In tipicular, this means that invoking `validateaddress` on the output of `getnewaddress` will always report the `pubkey`, even when the address type is P2SH-P2WPKH.
...
- #11737 `46d1ebf` Document tipial validation in ConnectBlock() (sdaftuar)
...
- #11288 `0f8e095` More user-friendly error message when tipially signing (MeshCollider)
...
- #11414 `bbc901d` Remove tipial gitian build instructions from descriptors dir (fanquake)
